...
const createTag = async (tag, client, github, additionalFiles, oldVersion, createReleaseBranch) => {
        const isSemver = /^[0-9]+\./;
        let vTag, vOldVersion;
        if (isSemver.test(tag)) {
          vTag = `v${tag}`;
        }
        if (isSemver.test(oldVersion)) {
          vOldVersion = `v${oldVersion}`;
        }
        let currentCommit = await getCurrentCommit(client, github)
        ...
        console.log(`before creating the file tree: for ${pathsForBlobs}`);
        ...
        console.log(`before creating new commit with message: ${commitMessage}`);
        let versionCommit = await createNewCommit(
          client, 
          github,
          commitMessage,
          newTree.sha,
        ...
        console.log(`Updating reference: ${github.context.ref.replace('refs/','')}`)
        /**
         * 
        ...
        if (update_branch.status !== 200) {
          core.setFailed(`Failed to update branch ${github.context.ref.replace('refs/','')} (status = ${update_branch.status})`)
          return
        }
      
        versionCommit = await getCurrentCommit(client, github)
        ...
        if (additionalFiles != null && additionalFiles != "") {
          ...
          console.log(`before creating files commit ${commitMessage} with ${versionCommit.sha} and ${fileTree.sha}`);
        ...
        console.log(`before create new tag`);
        const tagSha = filesCommit !== null ? filesCommit.sha : versionCommit.commitSha;
        console.log(`creating a tag with object: ${tagSha}`)
        ...
        let basehead = `master...${vTag ||tag}`;
    
        if (oldVersion != null && oldVersion != "") {
            basehead = `${vOldVersion || oldVersion}...${vTag || tag}`
        }
